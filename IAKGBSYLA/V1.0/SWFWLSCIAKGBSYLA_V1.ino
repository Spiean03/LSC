/***********************************************************************
  Programmer:
    ...
    Following Libraries are needed for Adafruit 2.2" TFT Breakout Card:
      ...
      - Adafruit_ILI9340
************************************************************************/
String versionNumber = "IAKGBSYLA, V1.0, Nov 2020";
...
void setup() {
  ...
  pinMode(PinOpenCollectorOutput3,OUTPUT); // Gate Valve Turbo Pump
...
void serialCommands() {
  if(Serial.available() > 0) {
    ...
    else if (message == "LSCModePump"){modeBooleanVent = false; menuMode(); Serial.println(1);}
    else if (message == "LSCModeVent"){modeBooleanVent = true; menuMode(); Serial.println(0);}
...
void menuTurboPump(){
  digitalWrite(PinOpenCollectorOutput1,LOW); //Turn off LN2 Dewar in case it is open (Safety Feature)
  Beeper(20);
  if (turboPumpBoolean == true) {
      //Turbo Pump ON
      tft.fillRect(2, 84, 155, 73, colorFont[settingsMemory[3]]);
      tft.setTextColor(colorBackground[settingsMemory[2]]);
      tft.setTextSize(2);
      tft.setCursor(15, 87);
      tft.println("TURBO PUMP");
      tft.setCursor(63, 140);
      tft.println("ON");  
      tft.setTextColor(colorFont[settingsMemory[3]]);
      digitalWrite(PinMOS3, HIGH); //Scroll Pump ON
  } else{
      //Turbo Pump OFF:
      tft.fillRect(2, 84, 155, 73, colorBackground[settingsMemory[2]]);
      tft.setTextColor(colorFont[settingsMemory[3]]);
      ...
      tft.setCursor(55, 140);
      tft.println("OFF");
      digitalWrite(PinMOS3, LOW); //Scroll Pump OFF
  }
}
...
//      }
      ...
      while(waitTimer <=20 && requestGranted ==false){
        ReadOut12bit = ADC12bit(PinPressureArray[0]); 
        if(ReadOut12bit >= 3000 || ReadOut12bit <= 256){//Pressure in Dock >=1mbar
     ...
     while(waitTimer <=20 && requestGranted ==false){
        ...
        if(ReadOut12bit >= 3000 || ReadOut12bit <= 256){//Pressure in Chamber >=1mbar
          requestGranted = false;
        } else{ //Pressure in Dock <=1mbar
          requestGranted =true;
        }
...
void menuGateValve() {
  ...
  if (gateValveBoolean == true){ //OPEN the Gate Valve
      ...
      if (modeBooleanVent == true){ // if Mode is set to "VENTED", do not open the Gate Valve
