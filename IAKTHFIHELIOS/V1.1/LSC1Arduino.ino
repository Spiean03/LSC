/***********************************************************************
  Programmer:
    ...
    Following Libraries are needed for Adafruit 2.2" TFT Breakout Card:
      ...
      - Adafruit_ILI9340
************************************************************************/
String versionNumber = "IAKTHFIHELIOS, V1.1, Aug 2020";
...
//Analog Pins - Not defined yet (can be used for Pressure or Temperature Readout)
...
#define PinPowerSwitch1 37 //D37, Vent Valve OPEN
#define PinPowerSwitch2 38 //D38, Angle Valve OPEN
#define PinPowerSwitch3 39 //D39, Gate Valve OPEN
#define PinPowerSwitch4 40 //D40, Gate Valve CLOSE

// Pins Open Collector Output 40V, ca. 30mA
#define PinOpenCollectorOutput1 41 //D41
#define PinOpenCollectorOutput2 42 //D42
#define PinOpenCollectorOutput3 43 //D43
...
#define PinMOS3 36 //D36 "0"=Open, Transfer Request for SEM
...
String menuItems[menuSize]={"Welcome Screen","Main Menu","Settings Menu 1","Settings Menu 2"};
int currentMenu =0;
int previousMenu =0;
...
boolean scrollPumpBoolean = true;
...
void setup() {
  ...
  pinMode(PinPowerSwitch1, OUTPUT); //Vent Valve OPEN
  pinMode(PinPowerSwitch2, OUTPUT); // Angle Valve OPEN
  ...
  pinMode(PinMOS3,OUTPUT); // Transfer Request for SEM
...
void serialCommands() {
  if(Serial.available() > 0) {
    ...
    currentMenu = 1;
    if (message == "LSCDebugOff"){
      settingsMemory[5] = 0;
      debug = false;
      if(modeBoolean == true && gateValveBoolean == true){
        modeBoolean = false;
      }
      displayMainMenu();
      //menuHeater();
    ...
    else if (message == "LSCGateValveClose"){gateValveBoolean = false; menuGateValve(); Serial.println(0);}
    else if (message == "LSCGateValveOpen"){gateValveBoolean = true; menuGateValve(); Serial.println(1);}
    else if (message == "LSCModePump"){modeBoolean = false; menuMode(); Serial.println(1);}
    else if (message == "LSCModeVent"){modeBoolean = true; menuMode(); Serial.println(0);}
...
void menuMode() {
  ...
  if (modeBoolean == true) {//Vent
      ...
      digitalWrite(PinPowerSwitch2,LOW); //Angle Valve CLOSE
      delay(500);
      ...
      float ReadOut12bit;
      ReadOut12bit = ADC12bit(PinPressureArray[0]); // Read the pressure from the pressure gauge
      if (scrollPumpBoolean == false){
        tft.fillRect(2, 190, 155, 15, colorFont[settingsMemory[3]]);
        tft.setTextColor(colorBackground[settingsMemory[2]]);
        tft.setTextSize(1);
        tft.setCursor(25, 195);
        tft.println("Turn On Scroll Pump");
        Beeper(2000);
        tft.fillRect(2, 190, 155, 15, colorFont[settingsMemory[3]]);
        modeBoolean = true;
        tft.setTextColor(colorFont[settingsMemory[3]]);
      ...
      tft.setCursor(50, 167);
      tft.println("MODE"); 
      digitalWrite(PinPowerSwitch2,HIGH); //Angle Valve OPEN
      tft.setCursor(40, 220);
      tft.println("PUMPED");
...
void menuGateValve() {
  ...
  if (gateValveBoolean == true){ //OPEN the Gate Valve
      ...
      if(digitalRead(PinDigitalInIsolated6) == false){ //during startup, if the gate valve is already detected to be open, keep it open
        ...
        if (debug == true){
          tft.setTextSize(1);
          tft.setCursor(213, 118);
          tft.println("DEBUG ON");
        }              
        tft.setTextColor(colorFont[settingsMemory[3]]);
        return;
      }

      if (debug == false && modeBoolean == true){ // if Mode is set to "VENTED", do not open the Gate Valve
        tft.fillRect(185, 112, 155, 15, colorBackground[settingsMemory[2]]);
        tft.setTextSize(1);
        tft.setCursor(185, 118);
        tft.println("Change Mode to PUMP");
        tft.setTextSize(2);
        Beeper(2000);
        gateValveBoolean = false;
        menuGateValve();
      ...
      if (debug == false && pressureInRange == false){ // Only open the Gate Valve if the Pressure is in Range <256 equals Gauge Error, >3350 is above Pressure Threshold for FIB/SEM
        //tft.fillRect(2, 190, 155, 15, colorFont[settingsMemory[3]]);
        tft.fillRect(185, 113, 155, 15, colorBackground[settingsMemory[2]]);
        tft.setTextSize(1);
        tft.setCursor(190, 118);
        tft.println("Pressure not OK");
        tft.setTextSize(2);
        Beeper(2000);
        gateValveBoolean = false;
        menuGateValve();
      ...
      else if (digitalRead(PinDigitalInIsolated6) == true){ //If gate Valve is detected to be closed
        if (settingsMemory[5]==0){  //If Safety Mode == ON
          
          tft.fillRect(164, 113, 130, 15, colorFont[settingsMemory[3]]);
          tft.setTextSize(1);
          tft.setCursor(220, 118);
          tft.println("SEM Prepare");
          if(digitalRead(PinDigitalInIsolated2)==HIGH){ //If "Gate Valve Allow" from FIB is denied, send a request
            tft.fillRect(175, 113, 130, 15, colorFont[settingsMemory[3]]);
            tft.setTextSize(1);
            tft.setCursor(180, 118);
            tft.println("Request for Loading");
            digitalWrite(PinMOS3,HIGH); //Transfer Request Closed (for at least 0.5Sec)
            delay(1500);
            digitalWrite(PinMOS3,LOW);
            tft.fillRect(175, 113, 130, 15, colorFont[settingsMemory[3]]);
            tft.setCursor(205, 118);
            tft.println("Request Sent");
            int waitTimer =0;
            boolean requestGranted =false;
            while(waitTimer <=10 && requestGranted ==false){ 
              if(digitalRead(PinDigitalInIsolated2)==HIGH){ //If GV Allow from SEM = 0V
                requestGranted = false;
              } else{ //If GV Allow from SEM = 24V ->Permission Granted
                requestGranted =true;
              }
              tft.setTextColor(colorFont[settingsMemory[3]]);
              MeasureTemperature();
            ...
            if(requestGranted == false){ //Permission not Granted
              tft.fillRect(175, 113, 130, 15, colorFont[settingsMemory[3]]);
              tft.setTextSize(1);
              tft.setCursor(205, 118);
          ...
          tft.fillRect(175, 113, 130, 15, colorFont[settingsMemory[3]]);
          tft.setTextSize(1);
          tft.setCursor(202, 118);
          tft.println("Opening Valve");
          delay(1000);
          tft.fillRect(164, 113, 130, 15, colorFont[settingsMemory[3]]);
          digitalWrite(PinPowerSwitch4, false); //Close Gate Valve OFF
          digitalWrite(PinPowerSwitch2, false); //Close Angle Valve
          ...
          delay(3000);
          digitalWrite(PinPowerSwitch3, false); //Open Gate Valve OFF
        } else if (settingsMemory[5]==1){ // If Safety Mode = OFF
          digitalWrite(PinPowerSwitch4, false); //Close Gate Valve OFF
          digitalWrite(PinPowerSwitch2, false); //Close Angle Valve
          delay(100);
          digitalWrite(PinPowerSwitch3, true); //Open Gate Valve ON
      ...
      if (debug == true){
        ...
        tft.setCursor(213, 118);
        tft.println("DEBUG ON");
      } else if(settingsMemory[5] == 0){ //SEM Interaction ON
          tft.setTextSize(1);
          tft.setCursor(190, 118);
          tft.println("SEM Interact: ON");
      } else{  //SEM Interaction OFF
          tft.setTextSize(1);
          tft.setCursor(190, 118);
          tft.println("SEM Interact: OFF");
      }
      ...
      if(IsButtonPressedTimer >=10){
        ...
        delay(1000);
        digitalWrite(PinPowerSwitch4, false); //Close Gate Valve OFF
        digitalWrite(PinPowerSwitch2, true); //Open Angle Valve
        if (debug == true){
          tft.fillRect(208, 113, 100, 20, colorBackground[settingsMemory[2]]);
          tft.setTextSize(1);
          tft.setCursor(213, 118);
          tft.println("DEBUG ON");
        } else if(settingsMemory[5] == 0){ //SEM Interaction ON
...
void MeasurePressure() {
  ...
  */
  ...
  for (int i = 0; i < 2 ; i++) {
    ...
    if (i==0){
        // Send the Pressure from the Preparation Chamber to the SEM: Signal Threshold Value =7.5V, above = it is allowed to open, below = it is not allowed to open
        int sendPressureToSEM = Value12bit*8.5/10;
        analogWrite(DAC0, sendPressureToSEM);
    }
    if (Value12bit <= 1024){
      ...
      if(i ==0){
        ...
        if(voltageConversion>8.5){
          voltageConversion =8.5;
        }
      }
      if(settingsMemory[0]==0){
        //mbar
          ...
          if(i==0){
            ...
            switch (currentMenu){
              case 2:
                ...
                tft.fillRect(55,125,100,30,colorBackground[settingsMemory[2]]);
                tft.setTextColor(colorFont[settingsMemory[3]]);
                tft.setCursor(60, 130);
                //tft.println(heaterTimingValuesDisplayed[settingsMemory[4]]);
                tft.println(safetyModeValuesDisplayed[settingsMemory[5]]);
      ...
      if (presentButtonStates[i] == HIGH){
        ...
        while(digitalRead(PinButtonArray[i])==HIGH && buttonPressTimer <=5){
          ...
          if(i==1){
            ...
            if(tftBoolean == true){// turn off display
              digitalWrite(PinDisplayBL,LOW);
              tftBoolean = false;
              return;
            } else{// turn on display
              digitalWrite(PinDisplayBL,HIGH);
              tftBoolean = true;
              return;
              }
